                         ...
                         AVInputFormat *fmt, AVFormatParameters *ap)
{
    ...
    if (ic->iformat->read_header) {
        err = ic->iformat->read_header(ic, ap);   //libavformat/mpegts.c  mpegts_read_header()
        if (err < 0)
            goto fail;
    }
                       ...
                       AVFormatParameters *ap)
{
    ...
    ByteIOContext *pb = NULL;   //这个结构体很重要，读取的文件原始数据就存放在这个结构体中
    void *logctx= ap && ap->prealloced_context ? *ic_ptr : NULL;

    pd->filename = ""; //
    if (filename)
        pd->filename = filename;   //此时这个结构体已经获得了输入文件的名字
    pd->buf = NULL;  //存放读取的文件数据
    pd->buf_size = 0;
    ...
    if (!fmt || !(fmt->flags & AVFMT_NOFILE)) {  //执行
        /* if no file needed do not try to open one */
        if ((err=url_fopen(&pb, filename, URL_RDONLY)) < 0) {  //很重要，在里面打开文件，读取数据  libavformat/aviobuf.c,信息都存放在pb中
            goto fail;
        }
        if (buf_size > 0) {
...
int av_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for(;;){
        ...
        av_init_packet(pkt);
        ret= s->iformat->read_packet(s, pkt);  //开始读取ts包的数据，之前是处理整个ts流，现在是一个一个的ts包数据
...
static int av_read_frame_internal(AVFormatContext *s, AVPacket *pkt)
{
    ...
    av_init_packet(pkt);  //初始化一个包

    for(;;) {
        /* select current input stream component */
        st = s->cur_st;
        if (st) {  //根据之前读取的包中包含的视频或音频数据帧是否处理完
            if (!st->need_parsing || !st->parser) {
                ...
                *pkt = st->cur_pkt; st->cur_pkt.data= NULL;
                ...
                len = av_parser_parse2(st->parser, st->codec, &pkt->data, &pkt->size,
                                       ...
                                       st->cur_pkt.pos);  //根据对应的解析器进行解析参数，一般是解析出一帧的数据
        ...
        } else {  //上面的if判断的是之前读取的一个ts包中的视频流或音频流是否处理完，若处理完，则进入下面的else中。否则继续解析
            AVPacket cur_pkt;
            /* read next packet */
...
int av_read_frame(AVFormatContext *s, AVPacket *pkt)
{
	...
	//读取完毕后，通过av_parser_parse2(***)分析出视频一帧（或音频若干帧），返回，
	//下次进入循环的时候，如果上次的数据没有完全取完，则st = s->cur_st;不会是NULL，
	//即再此进入av_parser_parse2(***)流程，而不是下面的av_read_packet（**）流程，
	//这样就保证了，如果读取一次包含了N帧视频数据（以视频为例），则调用av_read_frame（***）N次都不会去读数据，而是返回第一次读取的数据，直到全部解析完毕。
...
static int try_decode_frame(AVStream *st, AVPacket *avpkt)
{
    ...
    //知道了codec，但不知道媒体格式，调用try_decode_frame解码frame
    if(!st->codec->codec){
...
{
	//这个函数就是获得输入流的信息，并存放在AVFormatContext *ic 中。
    int i, count, ret, read_size, j;
    AVStream *st;
    AVPacket pkt1, *pkt;
    int64_t old_offset = url_ftell(ic->pb);

    for(i=0;i<ic->nb_streams;i++) {  //流的个数，可能有视频流 音频流  字幕流等
        AVCodec *codec;
        st = ic->streams[i];
...
/*            if(!st->time_base.num)
        ...
        codec = avcodec_find_decoder(st->codec->codec_id);   //找出对应的流所用的解码器，返回一个AVCodec
    ...
    for(;;) {
        ...
        /* NOTE: a new stream can be added there if no header in file
           (AVFMTCTX_NOHEADER) */
        ret = av_read_frame_internal(ic, &pkt1);  //读取一个包的信息，并解析出来
...
//                    av_log(NULL, AV_LOG_ERROR, "%f\n", dur);
        ...
        if (!has_codec_parameters(st->codec) || !has_decode_delay_been_guessed(st))
            try_decode_frame(st, pkt);   //找出这里到底为了什么信息去解码的，看能不能跳过这个解码过程。
...
int av_write_header(AVFormatContext *s)
{
    ...
    if(s->oformat->write_header){  //重要
        ret = s->oformat->write_header(s);  //libavformat/mpegtsenc.c  这是编写最终输出的ts文件的头信息，封装，目前是将相关信息填入到结构体中，并不是写文件
        if (ret < 0)
            return ret;
    }
