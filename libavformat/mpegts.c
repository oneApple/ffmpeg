...
static int get_packet_size(const uint8_t *buf, int size)
{
    ...
    score    = analyze(buf, size, TS_PACKET_SIZE, NULL);  //TS_PACKET_SIZE  188
    dvhs_score    = analyze(buf, size, TS_DVHS_PACKET_SIZE, NULL);   //TS_DVHS_PACKET_SIZE  192
    fec_score= analyze(buf, size, TS_FEC_PACKET_SIZE, NULL);    //TS_FEC_PACKET_SIZE   204
...
static int handle_packet(MpegTSContext *ts, const uint8_t *packet)
{
    ...
    //pid=0 节目源信息表PAT  pid=17 网络信息表NIT  pid=1 条件接收表CAT
    if(pid && discard_pid(ts, pid))
        return 0;
    is_start = packet[1] & 0x40;  //获得的是起始指示
    tss = ts->pids[pid];
    ...
    if (afc == 3) {
        ...
        p += p[0] + 1;   //跳过自适应区，如果有的话
    }
    /* if past the end of packet, ignore */
    p_end = packet + TS_PACKET_SIZE;
    if (p >= p_end)
        return 0;   //上面是测试包数据是否跳完了
    ...
    if (tss->type == MPEGTS_SECTION) {
        if (is_start) {
            ...
            if (len && cc_ok) {
            	...
            	//负载部分由A Section的End部分和B Section的Start组成，把A的End部分写入
...
static int handle_packets(MpegTSContext *ts, int nb_packets)
{
    ...
    for(;;) {
        ...
        //查找0x47，读取ts包大小的数据到packet中存放，一般为188
        if (ret != 0)
            return ret;
        ret = handle_packet(ts, packet);   //每次处理一个ts的包
        if (ret != 0)
            return ret;
    }
    return 0;
                              ...
                              AVFormatParameters *ap)
{
    ...
    pos = url_ftell(pb);   //获得位置信息，当前的指针，第一次为0  ，aviobuf.c
    len = get_buffer(pb, buf, sizeof(buf));   //len 为 5120
    if (len != sizeof(buf))
        goto fail;
    ts->raw_packet_size = get_packet_size(buf, sizeof(buf));  //获得ts包的大小，188/192/204
    ...
    if (s->iformat == &mpegts_demuxer) {
        ...
        mpegts_open_section_filter(ts, SDT_PID, sdt_cb, ts, 1);   //SDT_PID  17

        mpegts_open_section_filter(ts, PAT_PID, pat_cb, ts, 1);   //PAT_PID   0
        //上面的两个函数是打开PES和section的filter，会用到

        handle_packets(ts, s->probesize / ts->raw_packet_size);
        //handle_packets()  -->  read_packet()  -->  handle_packet()  -->  write_section_data()
