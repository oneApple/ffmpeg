...
int get_buffer(ByteIOContext *s, unsigned char *buf, int size)
{
    ...
    size1 = size;
    while (size > 0) {  //size 为1024，每次处理的数据大小
        len = s->buf_end - s->buf_ptr;   //buf_ptr指向当前数据的位置，len用来判断当前整个文件的数据还有多少没有读取出来
        if (len > size)
            len = size;
        if (len == 0) {
            if(size > s->buffer_size && !s->update_checksum){
                ...
                fill_buffer(s);  //根据文件指针，读取数据
                len = s->buf_end - s->buf_ptr;
                if (len == 0)
                    break;
            }
        } else {
            memcpy(buf, s->buf_ptr, len);  //将要处理的数据读取到buf中，读取的大小为len，一般为2048
            buf += len;
            s->buf_ptr += len;   //移动当前数据的指针
            size -= len;
        }
    }
    if (size1 == size) {
...
int url_fdopen(ByteIOContext **s, URLContext *h)
{
    ...
    buffer = av_malloc(buffer_size);  //分配临时缓冲区的空间
    if (!buffer)
    ...
    *s = av_mallocz(sizeof(ByteIOContext));
    ...
    if (init_put_byte(*s, buffer, buffer_size,
                      ...
                      url_read, url_write, url_seek) < 0) {  //赋值给s
        av_free(buffer);
        av_freep(s);
        return AVERROR(EIO);
    }
...
int url_fopen(ByteIOContext **s, const char *filename, int flags)
{
    ...
    err = url_open(&h, filename, flags);  //libavformat/avio.c  ，获得的相关信息和数据均在h这个结构体中
    if (err < 0)
        return err;
    err = url_fdopen(s, h);  //将h中的信息移到s中
    if (err < 0) {
        url_close(h);
        return err;
    }
